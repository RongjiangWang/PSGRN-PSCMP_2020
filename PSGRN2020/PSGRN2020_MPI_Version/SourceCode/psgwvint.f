     ...
     &                    nkc,nkmax,dk0,tty,duwk)
      use psgalloc
      implicit none
c
      integer*4 istate,lf,nr1,nr2,nkc,nkmax
      real*8 dk0
      complex*16 cs
      complex*16 duwk(nr,14,4)
      logical*2 tty
c
c     u: 1=uz, 2=ur, 3=ut,
...
c           toroidal mode (sh);
c
      integer*4 i,istp,ir,ik,nx,nkg,nnk,nlr,ierr
      real*8 k,dk,kg,kgr,dkg,krt
      real*8 pi,pi2,x,wl,wr,fac,y0abs,dyabs
      real*8 bsj0(-1:3),y0max(8,4)
      complex*16 c0,c1,c2,c3,c4,c05,ck
      complex*16 clazr,cmuzr,clahs,cmuhs,evs
      complex*16 cbsj(3)
      complex*16 y0(8,4),cy(14,4),obswk(nr,16,4),obs0wk(nr,16,4)
      complex*16 urdr(4),utdr(4),urdr0(4)
      logical*2 finish,tests
c
      integer*4 iret,ipot
      real*8 xokada,yokada,dip
      complex*16 alpha,pot1,pot2,pot3,pot4,cs45,ss45,potfac
      complex*16 ux,uy,uz,uxx,uyx,uzx,uxy,uyy,uzy,uxz,uyz,uzz
...
c
     ...
     &               /dsqrt(0.5d0*pi*x)
          ...
          do istp=1,4
            ...
            enddo
            obswk(ir,1,istp)=obswk(ir,1,istp)+cy(1,istp)*cbsj(2)
            obswk(ir,2,istp)=obswk(ir,2,istp)
     &         +cy(2,istp)*cbsj(1)-cy(3,istp)*cbsj(3)
            obswk(ir,3,istp)=obswk(ir,3,istp)
     &         -cics(istp)*(cy(2,istp)*cbsj(1)+cy(3,istp)*cbsj(3))
            obswk(ir,4,istp)=obswk(ir,4,istp)+cy(4,istp)*cbsj(2)
            obswk(ir,5,istp)=obswk(ir,5,istp)-cy(5,istp)*cbsj(2)
            obswk(ir,7,istp)=obswk(ir,7,istp)
     &          +cy(6,istp)*cbsj(1)-cy(7,istp)*cbsj(3)
            obswk(ir,8,istp)=obswk(ir,8,istp)+cy(8,istp)*cbsj(2)
            obswk(ir,9,istp)=obswk(ir,9,istp)
     &          -cics(istp)*(cy(6,istp)*cbsj(1)+cy(7,istp)*cbsj(3))
            obswk(ir,10,istp)=obswk(ir,10,istp)
     &          +c05*(cy(9,istp)*cbsj(1)-cy(10,istp)*cbsj(3))
            obswk(ir,11,istp)=obswk(ir,11,istp)-c05*cics(istp)
     &          *(cy(9,istp)*cbsj(1)+cy(10,istp)*cbsj(3))
            obswk(ir,12,istp)=obswk(ir,12,istp)+cy(11,istp)*cbsj(2)
            obswk(ir,13,istp)=obswk(ir,13,istp)+cy(12,istp)*cbsj(2)
            obswk(ir,14,istp)=obswk(ir,14,istp)
     &          +c05*(cy(13,istp)*cbsj(1)-cy(14,istp)*cbsj(3))
            obswk(ir,15,istp)=obswk(ir,15,istp)-c05*cics(istp)
     ...
     &               .le.accuracy*cdabs(urdr(istp))
          ...
          do istp=1,4
            do i=1,15
              ...
              dyabs=0.d0
              do ir=nr1,nr2
                y0abs=y0abs+geow(ir)*cdabs(obswk(ir,i,istp))
                dyabs=dyabs
     &               +geow(ir)*cdabs(obswk(ir,i,istp)-obs0wk(ir,i,istp))
                obs0wk(ir,i,istp)=obswk(ir,i,istp)
              enddo
...
c
          if(r(ir).le.0.d0)then
            obswk(ir,6,istp)=urdr(istp)
     &                    +cics(istp)*cms(istp)*utdr(istp)
          else
            obswk(ir,6,istp)=(obswk(ir,2,istp)+cics(istp)*cms(istp)
     &                    *obswk(ir,3,istp))/dcmplx(r(ir),0.d0)
          endif
c 
c         obs5 now is err = obs5(before) - ett
c
          obswk(ir,5,istp)=obswk(ir,5,istp)-obswk(ir,6,istp)
c
c         obs4 now is ezz
c
          obswk(ir,4,istp)=(obswk(ir,4,istp)
     &        -clazr*(obswk(ir,5,istp)+obswk(ir,6,istp)))
     &        /(clazr+c2*cmuzr)
c
c         obs7 now is ezr = erz
c
          obswk(ir,7,istp)=obswk(ir,7,istp)/(c2*cmuzr)
          obswk(ir,16,istp)=c05*obswk(ir,8,istp)
c
...
c
          if(r(ir).le.0.d0)then
            obswk(ir,8,istp)=obswk(ir,16,istp)
     &              -(cics(istp)*cms(istp)*urdr(istp)+utdr(istp))
          else
            obswk(ir,8,istp)=obswk(ir,16,istp)
     &                    -(cics(istp)*cms(istp)*obswk(ir,2,istp)
     &                    +obswk(ir,3,istp))/dcmplx(r(ir),0.d0)
          endif
c
c         obs9 now is ezt = etz
c
          obswk(ir,9,istp)=obswk(ir,9,istp)/(c2*cmuzr)
c
c         obs10 now is -dur/dz (vertical tilt)
c                      =  obs10(before) - 2 * ezr
c                      =  duz/dr - 2 * ezr
c
          obswk(ir,10,istp)= obswk(ir,10,istp)-c2*obswk(ir,7,istp)
        enddo
      enddo
c
...
c
          duwk(ir,1,istp)=obswk(ir,1,istp)
          duwk(ir,2,istp)=obswk(ir,2,istp)
          duwk(ir,3,istp)=obswk(ir,3,istp)
c
c         stress components
c
          evs=obswk(ir,4,istp)+obswk(ir,5,istp)+obswk(ir,6,istp)
          duwk(ir,4,istp)=clazr*evs+c2*cmuzr*obswk(ir,4,istp)
          duwk(ir,5,istp)=clazr*evs+c2*cmuzr*obswk(ir,5,istp)
          duwk(ir,6,istp)=clazr*evs+c2*cmuzr*obswk(ir,6,istp)
          duwk(ir,7,istp)=c2*cmuzr*obswk(ir,7,istp)
          duwk(ir,8,istp)=c2*cmuzr*obswk(ir,8,istp)
          duwk(ir,9,istp)=c2*cmuzr*obswk(ir,9,istp)
c
c         tilt components and rotation
c
          duwk(ir,10,istp)=obswk(ir,10,istp)
     &                 +obswk(ir,14,istp)/dcmplx(g0,0.d0)
          duwk(ir,11,istp)=obswk(ir,11,istp)-c2*obswk(ir,9,istp)
     &                 +obswk(ir,15,istp)/dcmplx(g0,0.d0)
          duwk(ir,12,istp)=obswk(ir,16,istp)
c
c         geoid
c
          duwk(ir,13,istp)=obswk(ir,12,istp)/dcmplx(g0,0.d0)
c
c         gravity change (space-fixed)
c
          duwk(ir,14,istp)=obswk(ir,13,istp)
        enddo
      enddo
c=============================================================================
...
c-----------------------------------------------------------------------
            urza=-uxz
c-----------------------------------------------------------------------
            duwk(ir,1,istp)=duwk(ir,1,istp)+uza
            duwk(ir,2,istp)=duwk(ir,2,istp)+ura
c
            duwk(ir,4,istp)=duwk(ir,4,istp)+szza
            duwk(ir,5,istp)=duwk(ir,5,istp)+srra
            duwk(ir,6,istp)=duwk(ir,6,istp)+stta
            duwk(ir,7,istp)=duwk(ir,7,istp)+szra

c
            duwk(ir,10,istp)=duwk(ir,10,istp)-urza
          enddo
c
c         strike-slip
c
...
c
          ...
          do ir=nr1,nr2
c
c           psv: z,r,t(Aki) = -z,-45 deg, -135 deg(Okada)
c
...
c-----------------------------------------------------------------------
c
c           updated on May 18,2024: start
c
c           erta=-uxy
c           srta=c2*cmuhs*erta
c
            srta=-cmuhs*(uxy+uyx)
c
c           updated on May 18,2024: end
c
            szta=cmuhs*(uyz+uzy)
c-----------------------------------------------------------------------
            utza=uyz
c-----------------------------------------------------------------------
            duwk(ir,3,istp)=duwk(ir,3,istp)+uta
c
            duwk(ir,8,istp)=duwk(ir,8,istp)+srta
            duwk(ir,9,istp)=duwk(ir,9,istp)+szta
c
            duwk(ir,11,istp)=duwk(ir,11,istp)-utza
            duwk(ir,12,istp)=duwk(ir,12,istp)-c05*(uyx-uxy)
          enddo
c
c         dip-slip
c
...
c
            duwk(ir,10,istp)=duwk(ir,10,istp)-urza
c
c            duwk(ir,14,istp)=duwk(ir,14,istp)
