...
c===================================================================
c     MPI task/result bookkeeping bridge
c===================================================================
c     These placeholders keep allocation and orchestration state in the
c     shared allocation module without depending on psgtask.f ordering.
      integer*4 mpi_rank,mpi_size
      data mpi_rank/0/,mpi_size/1/
      integer*4 ntasks,nresults
      data ntasks/0/,nresults/0/
      integer*8, allocatable:: task_id(:),result_id(:)
      integer*4, allocatable:: task_izs(:),task_isp(:)
      integer*4, allocatable:: task_nr1(:),task_nr2(:),task_status(:)
      integer*4, allocatable:: result_izs(:),result_isp(:)
      integer*4, allocatable:: result_nr1(:),result_nr2(:),result_status(:)
c$omp threadprivate(nongravity)
c$omp threadprivate(ms,ics,cics,cms)
c$omp threadprivate(sfct0,sfct1,sfcths0,sfcths1)
c$omp threadprivate(cla,cmu)
c$omp threadprivate(hkup,hklw)
c$omp threadprivate(maup,maiup,malw,mailw)
      end module
