...
c
          det1=dmax1(dabs(sxx-s(j)),dabs(sxy),dabs(szx))
          det2=dmax1(dabs(sxy),dabs(syy-s(j)),dabs(syz))
          det3=dmax1(dabs(szx),dabs(syz),dabs(szz-s(j)))
          ...
          if(det1.ge.am)then
            ...
            if(dabs(sxy).ge.am)then
              r(1,j)=-(syy-s(j)+syz)
              r(2,j)=sxy
              r(3,j)=sxy
            else if(dabs(syy-s(j)).ge.am)then
              r(1,j)=syy-s(j)
              r(2,j)=-(sxy+syz)
              r(3,j)=syy-s(j)
            else
            ...
            if(dabs(szx).ge.am)then
              r(1,j)=-(szz-s(j)+syz)
              r(2,j)=szx
              r(3,j)=szx
            else if(dabs(syz).ge.am)then
              r(1,j)=syz
              r(2,j)=-(szz-s(j)+szx)
              r(3,j)=syz
            else
